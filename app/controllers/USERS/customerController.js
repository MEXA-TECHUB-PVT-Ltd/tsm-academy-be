        ...
        if (email === null || email === "" || email === undefined) {
            ...
            if (userDataCheck.rows.length === 0) {
                ...
                const hashedPassword = crypto
                    ...
                    .digest("hex");
                const userData = await pool.query("INSERT INTO users(user_name,email,password,status) VALUES($1,$2,$3,$4) returning *",
                    [
                        user_name,
                        email,
                        hashedPassword,
                ...
                if (userData.rows.length === 0) {
                    ...
                    const userProgramData = await pool.query("INSERT INTO req_programs(user_id,program_id,package_id) VALUES($1,$2,$3) returning *",
                        [
                            user_id,
                            program_id,
                            package_id
                    ...
                    res.json({ error: false, data, dataProg, message: "User Created Successfully" });
                ...
                const dataProg = userProgramData.rows[0]
                res.json({ error: false, data, dataProg, message: "Course Added Successfully" });
        ...
        if (email === null || email === "" || email === undefined) {
            ...
            if (userDataCheck.rows.length === 0) {
                ...
                if (userData.rows.length === 0) {
                    ...
                    const user_id = data.user_id
                    const userDataCheckD = await pool.query("SELECT * FROM req_programs WHERE user_id=$1 AND program_id=$2 AND package_id=$3",
                        [user_id,
                            program_id,
                            package_id]);
                    if (userDataCheckD.rows.length === 0) {
                        ...
                        res.json({ error: false, data, password: password, dataProg, message: "User Created Successfully" });
                        // email send 
                        const subject = "Welcome Email"
                        Emailtemplate(user_name, password, email, subject)
                    } else {
                        res.json({ error: false, data: userDataCheckD.rows, message: "User Program Already Enrolled" });
                ...
                if (userDataCheckD.rows.length === 0) {
                    ...
                    if (userProgramData.rows.length === 0) {
                        res.json({ error: true, data, message: "Can't Create User Program" });
                        ...
                        res.json({ error: false, data, dataProg, message: "User Program Created Successfully" });
                        EmailLinkButton(user_name, email, subject)
    ...
    try {
        ...
        if (email === null || email === "" || email === undefined) {
            ...
            if (userDataCheck.rows.length === 0) {
                ...
                res.json({ error: false, otp: password, message: "Email Sent Successfully" });
...
exports.dashboardAdminCount = async (req, res) => {
    ...
    try {
        ...
        const resultContactResol = await pool.query(queryContactResol, ['resolved']);
        const lengthContactResol = resultContactResol.rows.length

        const queryContactReject = 'SELECT * FROM contact_us WHERE status=$1'
        const resultContactReject = await pool.query(queryContactReject, ['rejected']);
